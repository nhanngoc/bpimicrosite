...
@section('header')
    <div class="subheader">
        <h1 class="subheader-title">
            <i class='subheader-icon fal fa-usd-circle'></i> MDG Partner From SQL_SERVER
        </h1>
        <div class="subheader-block d-lg-flex align-items-center">
            <a href="{!! route('mdg-partner.index') !!}" class="btn btn-sm btn-secondary position-left">
                <i class="fal fa-exchange position-left"></i> Match Data MDG
            </a>
            <button type="button" class="btn btn-sm btn-secondary" data-toggle="modal" data-target=".example-modal-centered-transparent">
                <i class="ni ni-arrow-down position-left"></i>Update MDG DATA
            </button>
        </div>
    </div>
@stop
@section('content')
    <div id="panel-1" class="panel">
        <div class="panel-hdr">
            <h2>Last Update &nbsp;<i>{!! date('Y-m-d',strtotime($last->CHDAT)) !!} - {!! $count !!}
                    + {!! $count_mdg !!}</i></h2>
        </div>
        <div class="panel-container">
            <div class="panel-content">
                <table id="customer_vendors" class="table table-bordered table-hover table-striped w-100">
                    ...
                    <tr>
                        <th class="text-center" style="width: 120px;">PARTNER</th>
                        <th>BUKRS</th>
                        <th>BU_SORT1</th>
                        <th>NAME1</th>
                        <th>TEL_NUMBER</th>
    ...
    <div class="modal fade example-modal-centered-transparent" id="modalNew" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form id="fileUploadForm" method="POST" action="{{ route('mdg.process') }}">
                    @method('PUT')
                    @csrf
